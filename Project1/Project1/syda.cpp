...
int main() {
	...
	std::cout << std::endl;
	std::cout << "max sum 3x3: " << max_sum << std::endl;
	std::cout << "square 3x3:" << std::endl;
	for (int i = 0; i < 3; i++) {
		for (int j = 0; j < 3; j++) {
	...
	std::vector<std::vector<float>> matrix5x5(5, std::vector<float>(5));
	...
	for (int i = 0; i < 5; i++) {
		for (int j = 0; j < 5; j++) {
			...
			matrix5x5[i][j] = (float)sum3x3 / max_sum;
		}
	}
